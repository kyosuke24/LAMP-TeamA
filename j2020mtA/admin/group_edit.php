...
//$_POST優先
if(isset($_POST['Greet_id']) 
//cutilクラスのメンバ関数をスタティック呼出
	&& cutil::is_number($_POST['Greet_id'])
	&& $_POST['Greet_id'] > 0){
	$Greet_id = $_POST['Greet_id'];
}
//メンバークラスを構築
$greet_obj = new cGreet_table();
//配列にメンバーを$_POSTに取り出す
...
if(isset($_POST['func'])){
	...
	if($Greet_id > 0){
		if(($_POST = $greet_obj->get_tgt(false,$Greet_id)) === false){
			$Greet_id = 0;
			$_POST['func'] = 'new';
		}
		else{
			$_POST['Greet_pass_conf'] = $_POST['Greet_pass'];
			$_POST['func'] = 'edit';
		}
	}
	else{
...
function paramchk(){
	...
	$retflg = true;
	/// ユーザー名チェック
	if(ccontentsutil::chkset_err_field($err_array,'Greet_comment','団体挨拶','isset_nl')){
		$retflg = false;
    }
	...
	$ext_arr[] = '.png';
	$subdir = '';
	upload('Greet_pass',$ext_arr,$subdir,'main');
...
function regist(){
	global $Greet_id;
	$dataarr = array();
    $dataarr['Greet_comment'] = (string)$_POST['Greet_comment'];
    $dataarr['Greet_pass'] = (string)$_POST['Greet_pass'];
	$chenge = new cchange_ex();
	if($Greet_id > 0){
		$chenge->update('Greet_table',$dataarr,'Greet_id=' . $Greet_id);
		cutil::redirect_exit('group.php' . '?uid=' . $Greet_id);
	}
}
//--------------------------------------------------------------------------------------
/*!
...
function assign_Greet_id(){
	//$smartyをグローバル宣言（必須）
	global $smarty;
	global $Greet_id;
	$smarty->assign('Greet_id',$Greet_id);
}

/////////////////////////////////////////////////////////////////
/// 関数呼び出しブロック
/////////////////////////////////////////////////////////////////
if(!isset($_POST['Greet_comment']))$_POST['Greet_comment'] = '';
if(!isset($_POST['Greet_pass']))$_POST['Greet_pass'] = '';
...
if(isset($_POST['func']) && $_POST['func'] == 'conf'){
    ...
    $smarty->display('admin/group_edit_con.tmpl');
}
else{
    $smarty->display('admin/group_edit.tmpl');
}
?>