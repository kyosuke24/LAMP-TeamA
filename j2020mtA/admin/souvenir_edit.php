...
function paramchk(){
	...
	$retflg = true;
	/// 記念品名チェック
	if(ccontentsutil::chkset_err_field($err_array,'Souvenir_name','記念品名','isset_nl')){
		$retflg = false;
	}
	/// ポイントの存在と空白チェック
	if(ccontentsutil::chkset_err_field($err_array,'Souvenir_points','必要ポイント','isset_num')){
		$retflg = false;
	}
	/// 記念品紹介文チェック
	if(ccontentsutil::chkset_err_field($err_array,'Souvenir_introduction','公開情報','isset_nl')){
		$retflg = false;
	}
	...
	$ext_arr[] = '.png';
	$subdir = '';
	upload('Souvenir_pass',$ext_arr,$subdir,'main');
...
function regist(){
	global $Souvenir_id;
	$dataarr = array();
	$dataarr['Souvenir_name'] = (String)$_POST['Souvenir_name'];
	$dataarr['Souvenir_points'] = (int)$_POST['Souvenir_points'];
	$dataarr['Souvenir_introduction'] = (String)$_POST['Souvenir_introduction'];
	$dataarr['Souvenir_pass'] = (string)$_POST['Souvenir_pass'];
	$chenge = new cchange_ex();
	if($Souvenir_id > 0){
		$chenge->update('Souvenir_table',$dataarr,'Souvenir_id=' . $Souvenir_id);
		cutil::redirect_exit('souvenir_list.php' . '?mid=' . $Souvenir_id);
	}
	else{
		$mid = $chenge->insert('Souvenir_table',$dataarr);
...
*/
//--------------------------------------------------------------------------------------
function assign_Souvenir_id_txt(){
	//$smartyをグローバル宣言（必須）
	global $smarty;
	global $Souvenir_id;
	if($Souvenir_id <= 0){
		$smarty->assign('Souvenir_id_txt','新規');
	}
	else{
		$smarty->assign('Souvenir_id_txt',$Souvenir_id);
	}
}
...
if(isset($_POST['func']) && $_POST['func'] == 'conf'){
    ...
    $smarty->display('admin/souvenir_edit_con.tmpl');
}
else{
    $smarty->display('admin/souvenir_edit.tmpl');
}
